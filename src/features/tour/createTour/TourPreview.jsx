...
const TourPreview = (props) => {
  ...
  const renderLive = (index, style, user) => {
    switch (index) {
      case 0:
        const trailer = tour.all_media
          ? tour.all_media.find((media) => media.type.includes("video"))
          : null;
        if (trailer) {
          return (
            <video
          ...
          return <img className="img" src={defaultVideo} alt="defaultVideo" />;
        }
        break;
        if (false) {
          return (
        ...
        return (
          <div className="topHeader">
            <div>
              ...
              <div className="tourMainSentense">
                {tour.main_sentense && tour.main_sentense}
              </div>
            </div>
            <div>
  ...
  const arrSize = new Array(10)
    .fill(true)
    .map(() => 75 + Math.round(Math.random() * 50)); //new Array([200,200,200,200, 200, 200])
  const sizeArr = [200, 220, 220, 220, 260, 220, 150];
  ...
  const screen = () => (
    <List
      //className="List"
      height={getWindowDimensions() / 1.55}
      itemCount={7}
  ...
  return (
    <div className="previewArea">
      {tour && <div className="tourPreview">{screen()}</div>}
    </div>
  );
};
