...
const BusinessItem = (props) => {
    ...
    const tagByType = (item) => {
        ...
        if (item.type.includes('image')) {
            return <Grid.Column centered> <img width="180" hight="180" src={item.url} /></Grid.Column>;
        } else if (item.type.includes('audio')) {
            return (<Grid.Column><audio width="320" height="180" controls  >
                <source src={item.url} type={item.type} />
            </audio>
            </Grid.Column>
            );
        } else if (item.type.includes('video')) {
            return (<Grid.Column><video poster={item.poster_url} width="260" hight="180" controls>
                <source src={item.url} type={item.type} />
            </video>
            </Grid.Column>
            );
            ...
            <div>
                ...
                <div className="businessItemTop">
                    <div className='oneUnit'>
                        ...
                        {business.age_range &&<div className='businessName'>{business.age_range.min} - {business.age_range.max}</div>}
                    </div>
                    <div className='oneUnit'>
                    <div className="businessItemHeader">Available hours:</div>
                        {business.hours_range  &&<div className='businessName'>{formatHours(business.hours_range.min)} - {formatHours(business.hours_range.max)}</div>}
                    </div>
