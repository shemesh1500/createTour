...
const SmallStopMedia = (props) => {
    ...
    const tagByType = (item) => {
        if (item.type.includes('image')) {
            return <Image src={item.url} style={{height: '250px', width: '250px'}}/>;
    ...
    return (
        <Fragment>
            ...
            {initialValues.all_media && initialValues.all_media[0] &&
                <div>
                    <button className='saveButton' onClick={() => deleteFile(initialValues.all_media[0])}>Change context</button>
                </div>
            }
