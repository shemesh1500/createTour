...
export const generalUploadFile = (file, ownerId, collectionName) =>
    async (dispatch, setState, { getFirebase }) => {
        ...
        try {
            dispatch(asyncActionStart())
            let uploadedFile = await firebase.uploadFile(`${ownerId}/${collectionName}Media/`, file, null, options)
            let downloadURL = await uploadedFile.uploadTaskSnapshot.ref.getDownloadURL();
            let new_media = {
                name: fileName,
                url: downloadURL,
...
export const generalDeleteFile = (file, objectId, objectName) =>
